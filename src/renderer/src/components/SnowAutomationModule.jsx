...
export default function SnowAutomationModule({ D }) {
  ...
  const [portalOrigin, setPortalOrigin] = useState(
    'https://nordexprod.service-now.com/bam?id=external_portal_home'
  )
  ...
  useEffect(() => {
    ...
    let cancelled = false
    const poll = async () => {
      const tabs = await window.pywebview.api
  ...
  const loadWindfarmConfigs = async () => {
    const list = await window.pywebview.api
  ...
  const handleRunFullAutomation = async mode => {
    ...
    try {
      ...
      if (res.stopped) {
        ...
        setLogs(prev => [
          ...prev,
          { text: `Falha: ${res.error}`, type: 'error' },
        ])
      }
    } catch (err) {
      setLogs(prev => [
        ...prev,
        { text: `Erro crítico: ${err.message || err}`, type: 'error' },
      ])
    } finally {
  ...
  const handleLogin = async () => {
    ...
    setLogs(prev => [
      ...prev,
      { text: `Abrindo navegador em: ${incidentUrl}`, type: 'info' },
    ])
    try {
      const res = await window.pywebview.api.snow_automation_login(
        incidentUrl.trim()
      )
      if (res.success) {
        setLogs(prev => [
        ...
        setLogs(prev => [
          ...prev,
          { text: `Falha ao abrir navegador: ${res.error}`, type: 'error' },
        ])
      }
    } catch (err) {
      setLogs(prev => [
  ...
  const handleCloseSession = async () => {
    ...
    setLogs(prev => [
      ...prev,
      { text: 'Sessão do navegador encerrada.', type: 'info' },
    ])
  }
  ...
  const handleRun = async () => {
    ...
    try {
      const res = await window.pywebview.api.snow_automation_run(
        excelPath,
        incidentUrl.trim(),
        options
      )
  ...
  const handleAddToQueue = () => {
    ...
    setLogs(prev => [
      ...prev,
      {
        text: `Turbina adicionada à fila: ${item.label} (${item.bladeCount} pá(s)).`,
        type: 'info',
      },
    ])
    resetTurbineForm()
  ...
  const handleRunQueue = async () => {
    ...
    setLogs(prev => [
      ...prev,
      {
        text: `Iniciando fila overnight com ${queue.length} turbina(s)...`,
        type: 'info',
      },
    ])
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      setLogs(prev => [
        ...prev,
        {
          text: `\n===== Turbina ${i + 1}/${items.length}: ${item.label} =====`,
          type: 'info',
        },
      ])
      ...
      try {
        ...
        setQueue(prev =>
          prev.map(q =>
            q.id === item.id
              ? { ...q, status: res.success ? 'done' : 'failed', result: res }
              : q
          )
        )
        if (res.stopped) {
          setLogs(prev => [
            ...prev,
            {
              text: `Fila parada pelo usuário na turbina ${i + 1}/${items.length}: ${res.processed} ok, ${res.failed} falha(s).`,
              type: 'warning',
            },
          ])
          break
        ...
        if (res.success && res.dryRun) {
          ...
          setLogs(prev => [
            ...prev,
            {
              text: `Turbina ${i + 1}/${items.length} falhou: ${res.error} — seguindo para a próxima.`,
              type: 'error',
            },
          ])
        }
        ...
        setQueue(prev =>
          prev.map(q =>
            q.id === item.id
              ? { ...q, status: 'failed', result: { error: msg } }
              : q
          )
        )
        setLogs(prev => [
          ...prev,
          {
            text: `Turbina ${i + 1}/${items.length} erro crítico: ${msg} — seguindo para a próxima.`,
            type: 'error',
          },
        ])
      }
    ...
    setLogs(prev => [
      ...prev,
      {
        text: `Fila overnight concluída — ${items.length} turbina(s) processada(s).`,
        type: 'success',
      },
    ])
  }
    ...
    <div style={{ display: 'flex', gap: '18px', height: '100%', minHeight: 0 }}>
      ...
      >
        ...
        >
          ...
          <div className="form-group" style={{ margin: 0 }}>
            <div
              ...
              style={{ color: D.textMuted, fontSize: '12px' }}
            >
            ...
            >
              <span
                style={{
                  display: 'flex',
                  color: controlXlsxPath ? accent : D.textMuted,
                  flexShrink: 0,
                }}
              >
                {Icons.file(controlXlsxPath ? accent : D.textMuted)}
              </span>
              <span
                className="input-field-text"
                title={controlXlsxPath || 'Selecione o arquivo .xlsx'}
                style={{ fontSize: '12.5px' }}
              >
                {controlXlsxPath
                  ? controlXlsxPath.split('\\').pop()
          ...
          <div className="form-group" style={{ margin: 0 }}>
            <div
              ...
              style={{ color: D.textMuted, fontSize: '12px' }}
            >
              Pasta raiz das turbinas (ex: D:\SNOW\WTG'S)
            </div>
            <div
            ...
            >
              <span
                style={{
                  display: 'flex',
                  color: wtgRootFolder ? accent : D.textMuted,
                  flexShrink: 0,
                }}
              >
                {Icons.folder(wtgRootFolder ? accent : D.textMuted)}
              </span>
              <span
                className="input-field-text"
                title={wtgRootFolder || 'Selecione a pasta raiz'}
                style={{ fontSize: '12.5px' }}
              >
          ...
          >
            <input
              ...
              disabled={busy}
            />
            Pular turbinas com Status SNOW já "Enviado..." na planilha de
            controle
          </label>
        ...
        {busy && (
          <div style={{ display: 'flex', gap: '8px' }}>
            ...
            >
              ...
              >
                {paused ? Icons.play('#fff') : Icons.pause(D.textPrimary)}{' '}
        ...
        >
          ...
          >
            {Icons.globe(D.textSecond)} Configuração por Parque{' '}
            {showWindfarmConfig ? '▲' : '▼'}
          </span>
        </button>
          ...
          >
                  ...
                  >
                    <div style={{ minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: '10px', color: D.textMuted }}>
                        {c.leader} · {c.technicians.join(', ')} · PO{' '}
                        {c.purchaseOrder || '—'}
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                      <button
                        onClick={() => startEditWindfarm(c)}
                        style={{
                          background: 'none',
                          border: 0,
                          color: accent,
                      ...
                      <button
                        onClick={() => handleDeleteWindfarmConfig(c.windfarm)}
                        style={{
                          background: 'none',
                          border: 0,
                          color: D.error,
            ...
            {wfEditing ? (
              ...
              >
                <input
                  type="text"
                  placeholder="Nome do parque (igual ao 'windfarm' da base de pás)"
                  value={wfName}
                  onChange={e => setWfName(e.target.value)}
                  disabled={wfEditing !== '__new__'}
                  style={{
                    padding: '7px 9px',
                    borderRadius: '6px',
                    border: `1px solid ${D.borderLight}`,
                ...
                <input
                  ...
                  placeholder="Líder"
                  value={wfLeader}
                  onChange={e => setWfLeader(e.target.value)}
                  style={{
                    padding: '7px 9px',
                    borderRadius: '6px',
                    border: `1px solid ${D.borderLight}`,
                ...
                <input
                  type="text"
                  placeholder="Técnicos (separados por vírgula)"
                  value={wfTechnicians}
                  onChange={e => setWfTechnicians(e.target.value)}
                  style={{
                    padding: '7px 9px',
                    borderRadius: '6px',
                    border: `1px solid ${D.borderLight}`,
                ...
                <input
                  ...
                  value={wfPurchaseOrder}
                  onChange={e => setWfPurchaseOrder(e.target.value)}
                  style={{
                    padding: '7px 9px',
                    borderRadius: '6px',
                    border: `1px solid ${D.borderLight}`,
                ...
                <div style={{ display: 'flex', gap: '6px' }}>
                  <button
                    onClick={handleSaveWindfarmConfig}
                    style={{
                      flex: 1,
                      background: accent,
                      border: 0,
              ...
              <button
                onClick={() => startEditWindfarm(null)}
                style={{
                  background: 'none',
                  border: `1px dashed ${D.borderLight}`,
                  color: accent,
                  borderRadius: '6px',
                  padding: '7px',
                  fontSize: '11.5px',
        ...
        >
          ...
          />
          Submissão Automática (desativado = apenas preenche, você revisa e
          submete)
        </label>
        ...
        >
          <input
            ...
            disabled={busy || dryRun}
          />
          Modo Flawless (só avança de pá quando tudo — defeito, blank e vídeo —
          estiver 100% ok; mais lento, ideal pra overnight)
        </label>
          ...
          >
            ...
            <div className="form-group">
              <div className="field-label" style={{ color: D.textMuted }}>
                Planilha SNOW (gerada pelo módulo 23)
              </div>
              <div className="form-input-row">
                ...
                >
                  <span
                    style={{
                      display: 'flex',
                      color: excelPath ? accent : D.textMuted,
                      flexShrink: 0,
                    }}
                  >
                    {Icons.file(excelPath ? accent : D.textMuted)}
                  </span>
                  <span
                    className="input-field-text"
                    title={excelPath || 'Selecione o arquivo .xlsx'}
                  >
                    {excelPath
                      ? excelPath.split('\\').pop()
                      : 'Selecione o arquivo .xlsx'}
            ...
            <div className="form-group">
              <div className="field-label" style={{ color: D.textMuted }}>
                Pasta de Fotos Geradas (Módulo 23 - Fotos/)
              </div>
              <div className="form-input-row">
                <div
                ...
                >
                  <span
                    style={{
                      display: 'flex',
                      color: localPhotosDir ? accent : D.textMuted,
                      flexShrink: 0,
                    }}
                  >
                    {Icons.photos(localPhotosDir ? accent : D.textMuted)}
                  </span>
                  <span
                    className="input-field-text"
                    title={
                      localPhotosDir || 'Selecione a pasta Fotos/ (opcional)'
                    }
                  >
                    {localPhotosDir
                      ? localPhotosDir.split('\\').pop()
                      : 'Selecione a pasta Fotos/ (opcional)'}
                  </span>
                </div>
              </div>
            </div>
            ...
            {excelPath && (
              ...
              >
                ...
                >
                  <span style={{ fontWeight: 600, color: D.textPrimary }}>
                    Pás Encontradas na Planilha:
                  </span>
                  <div style={{ display: 'flex', gap: '6px' }}>
                    <button
                      type="button"
                ...
                {loadingBlades ? (
                  ...
                  >
                    {blades.map(b => {
                      const isChecked = selectedBlades.includes(
                        b.bladeSerialNumber
                      )
                      return (
                        <label
                        ...
                        >
                          ...
                          <span
                            style={{ color: D.textMuted, fontSize: '10px' }}
                          >
            ...
            <div className="form-group">
              <div className="field-label" style={{ color: D.textMuted }}>
                URL do Inspection Report (Incidente)
              </div>
              <input
                type="text"
                value={incidentUrl}
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <button
                ...
                style={{
                  ...
                  fontWeight: 500,
                  cursor:
                    loggingIn || busy || !incidentUrl.trim()
                      ? 'not-allowed'
                      : 'pointer',
              ...
              >
                {loggingIn ? (
                  ...
                  >
                    {Icons.key(D.textPrimary)} Abrir p/ Login
                  </span>
                )}
              </button>
              <button
            ...
            <div className="form-group">
              <div className="field-label" style={{ color: D.textMuted }}>
                Faixa de linhas filtradas (opcional)
              </div>
              <div style={{ display: 'flex', gap: '8px' }}>
                <input
                  type="number"
            ...
            >
              ...
              >
                <input
                  ...
                  disabled={busy}
                />
                Modo Auditoria (dry run — só verifica o que falta, não preenche
                nada)
              </label>
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          border: `1px solid ${D.borderLight}`,
                          background:
                            idx === queueIndex ? `${accent}10` : 'transparent',
                        }}
                      >
                        <div style={{ minWidth: 0 }}>
                          <div
                          ...
                          <div style={{ color: D.textMuted, fontSize: '10px' }}>
                            ...
                            {q.status === 'done' && q.result
                              ? ` — ${q.result.processed} ok, ${q.result.failed} falha(s)`
                              : ''}
                            {q.status === 'failed' && q.result?.error
                              ? ` — ${q.result.error}`
                              : ''}
                          </div>
                        </div>
            ...
            >
              ...
              >
                ...
                >
                  {Icons.plus(accent)} Adicionar à Fila
                </span>
              </button>
              ...
              <button
                ...
                style={{
                  background:
                    busy || selectedBlades.length === 0 ? D.bgHover : accent,
                  color: '#fff',
                  border: 0,
                  borderRadius: '8px',
                  padding: '10px',
              ...
              >
                {running ? (
                  ...
                  >
                    {Icons.play('#fff')} Rodar Agora ({selectedBlades.length}{' '}
                    pá(s))
                  </span>
                )}
              </button>
        ...
        >
          {result ? (
            <div style={{ display: 'flex', gap: '16px', fontSize: '12px' }}>
              <span style={{ color: D.success, fontWeight: 600 }}>
                {result.processed ?? 0} ok
              </span>
              <span
                style={{
                  color: result.failed ? D.error : D.textMuted,
                  fontWeight: 600,
                }}
              >
            ...
            >
              {openTabs.map(t => (
                ...
                >
                  ...
                  >
                    {t.purpose === 'video-review'
                      ? Icons.video(D.textSecond)
                      : Icons.fileText(D.textSecond)}{' '}
                    {t.turbine || ''} {t.blade ? `· ${t.blade}` : ''} ·{' '}
                    {t.label} · {tabAge(t.openedAt)}
                  </span>
                  <button
